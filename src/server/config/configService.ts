...
export interface SocketConfig {
    ...
    remoteStatusInactivityMsPlaying: number;
    remoteStatusInactivityMsPaused: number;
...
export class ConfigService {
    ...
    getString(key: ConfigKey, fallback?: string): string | undefined {
        ...
        if (this.cache.has(cacheKey)) return this.cache.get(cacheKey) as string | undefined;
    ...
    getNumber(key: ConfigKey, fallback?: number): number | undefined {
        ...
        if (this.cache.has(cacheKey)) return this.cache.get(cacheKey) as number | undefined;
    ...
    requireNumber(key: ConfigKey): number {
        ...
        if (typeof value !== 'number') throw new Error(`Missing or invalid numeric config: ${key}`);
        return value;
    }
    ...
    getSocketConfig(): SocketConfig {
        return {
            ...
            remoteStatusInactivityMsPlaying: this.getNumber(
                'REMOTE_STATUS_INACTIVITY_MS_PLAYING',
                this.getNumber('REMOTE_STATUS_INACTIVITY_MS', 0) ?? 0,
            ) ?? 0,
            remoteStatusInactivityMsPaused: this.getNumber(
                'REMOTE_STATUS_INACTIVITY_MS_PAUSED',
                this.getNumber('REMOTE_STATUS_INACTIVITY_MS', 0) ?? 0,
            ) ?? 0,
