...
export function extractMetaFields(
    ...
    if (
        ...
        if (!extractedUrl) extractedUrl = extractStringField(s, 'musicId', 'musicTitle');
    ...
    for (const key of Object.keys(metaForPrint)) if (key !== consumedKey) rest[key] = metaForPrint[key];
