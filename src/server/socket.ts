...
import { attachUpgradeRewrite, registerEngineAugmentations } from "./socket/engine";
...
export class SocketServerInstance {
    ...
    private remoteStatusDebounceMs = Number(SERVER_ENV.REMOTE_STATUS_DEBOUNCE_MS || 250);
    private remoteStatusGraceMs = Number(SERVER_ENV.REMOTE_STATUS_GRACE_MS || 5000);
    private remoteStatusInactivityMs = Number(SERVER_ENV.REMOTE_STATUS_INACTIVITY_MS || 1000 * 60 * 10);
    private timerManager = new TimerManager();
    private windowCloseManager = new WindowCloseManager(Number(SERVER_ENV.WINDOW_CLOSE_DEBOUNCE_MS || 500));
    ...
    static isEngineLike(v: unknown): v is EngineLike {
        ...
        return typeof rec["on"] === "function" || typeof rec["httpServer"] !== "undefined";
    }
    ...
    constructor(youtubeService?: YouTubeService, fileStore: Store = defaultFileStore) {
        this.youtubeService = youtubeService ?? new YouTubeService();
        this.fileStore = fileStore;
        this.adminHash = createHash("sha256").update(String(SERVER_ENV.ADMIN_SECRET)).digest("hex");
    }
    ...
    async init(server: HttpServer): Promise<void> {
        ...
        const candidatePrefixes = Array.from(new Set([socketPath, "/socket.io", "/api/socket.io"].filter(Boolean)));
        ...
        try {
            try {
                const addr = (server as unknown as { address?: () => unknown }).address?.();
        ...
        const makeConnectionHandler = mod.default as unknown as AnyConnectionHandler;
                ...
                if (!this.manager) {
                    this.manager = new SocketManager(
                        (ev: string, payload: unknown) => {
                            try {
                                const io = this.getIo();
                                (io.emit as unknown as (e: string, p: unknown) => void)(ev, payload);
                            } catch (e) {
    ...
    public emit(...args: Parameters<IOServer["emit"]>): ReturnType<IOServer["emit"]> | undefined {
        try {
            // Use getIo which guarantees initialized and returns a typed IOServer
            const ioServer = this.getIo();
            return (ioServer.emit as unknown as (...a: Parameters<IOServer["emit"]>) => ReturnType<IOServer["emit"]>)(
                ...args
            );
        } catch (e) {
    ...
    private getMusicService(): MusicService {
        if (!this.musicService) {
            this.musicService = new MusicService(this.musicDB, this.youtubeService, this.fileStore, (ev, payload) => {
                try {
                    const io = this.getIo();
                    (io.emit as unknown as (e: string, p: unknown) => void)(ev, payload);
                } catch (e) {
