...
export function registerMusicHandlers(io: Server<C2S, S2C>, socket: Socket<C2S, S2C>) {
    socket.on('adminAuth', (code: string, callback) => {
        ...
        if (!adminSecret) {
            ...
            if (typeof callback === 'function') callback({ success: false, error: '管理者認証が利用できません' });
            return;
        }
        ...
        if (code === adminSecret) {
            ...
            if (typeof callback === 'function') callback({ success: false, error: '無効な管理者コードです' });
        }
    });
    ...
    socket.on('adminAuthByQuery', (queryParam: string, callback) => {
        ...
        if (queryParam === adminSecret) {
            ...
            if (typeof callback === 'function') callback({ success: false, error: '無効な管理者クエリパラメータです' });
        }
    });

    socket.on('addMusic', (music: Music, callback) => {
        if (!musicActionLimit(socket.id)) {
            if (typeof callback === 'function') callback('楽曲追加が制限されています。1秒後にお試しください。');
            return;
        }
