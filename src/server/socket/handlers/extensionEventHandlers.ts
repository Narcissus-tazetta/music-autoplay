...
export function setupExtensionEventHandlers(
  ...
  registerSocketEventSafely(
    ...
    async (payload) => {
      ...
      if (stateRaw === "playing" || stateRaw === "paused") {
        ...
        if (url) {
          const { watchUrl, extractYoutubeId } =
            await import("@/shared/utils/youtube");
  ...
  registerSocketEventSafely(
    ...
    async (payload) => {
      ...
      try {
        const { extractYoutubeId, watchUrl } =
          await import("@/shared/utils/youtube");
        const videoId = extractYoutubeId(url);
  ...
  const handleProgressUpdate = async (payload: unknown, eventName: string) => {
    ...
    const playbackRate =
      typeof payload["playbackRate"] === "number" ? payload["playbackRate"] : 1;
    const isBuffering =
      typeof payload["isBuffering"] === "boolean"
        ? payload["isBuffering"]
        : false;
    ...
    const timestamp =
      typeof payload["timestamp"] === "number"
        ? payload["timestamp"]
        : Date.now();
    const isAdvertisementFromExtension =
      typeof payload["isAdvertisement"] === "boolean"
    ...
    try {
      ...
      if (isAdvertisementFromExtension !== undefined)
        isAdvertisement = isAdvertisementFromExtension;
