    ...
    return (socket: Socket) => {
        ...
        try {
            if (hs && typeof (hs as any).headers === "object") {
                const hdrs = (hs as { headers?: unknown }).headers as Record<string, unknown> | undefined;
                if (hdrs && typeof hdrs["x-request-id"] === "string") requestId = hdrs["x-request-id"];
            }
        } catch {
            requestId = undefined;
        }
        ...
        const origin =
            _headersSnapshot && typeof _headersSnapshot.origin === "string" ? _headersSnapshot.origin : undefined;
        ...
        try {
            logMetric("socketConnection", { clientSource, transport, isExtension }, { socketId: socket.id, origin });
        } catch (err) {
        ...
        if (isExtension) {
            ...
            if (connObj && typeof (connObj as ConnLike).on === "function") maybeConnOn = (connObj as ConnLike).on;

            if (typeof maybeConnOn === "function") {
                (maybeConnOn as (ev: string, handler: () => void) => void).call(connObj, "upgrade", () => {
                ...
                (maybeConnOn as (ev: string, handler: (err: unknown) => void) => void).call(
                    connObj,
                    "upgradeError",
                    (error: unknown) => {
                        logger.warn("extension websocket upgrade failed, continuing with polling", {
                            socketId: socket.id,
                            error,
                            timestamp: new Date().toISOString(),
                        });
        ...
        if (isExtension) {
            ...
            if (sockObj && typeof (sockObj as ConnLike).on === "function") maybeSockOn = (sockObj as ConnLike).on;

            if (typeof maybeSockOn === "function") {
                (maybeSockOn as (ev: string, handler: (data: unknown) => void) => void).call(
                    sockObj,
                    "extension_heartbeat",
                    (data: unknown) => {
                        try {
