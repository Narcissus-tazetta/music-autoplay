...
export function attachUpgradeRewrite(server: HttpServer, socketPath: string, candidatePrefixes: string[]): void {
    try {
                ...
                for (const p of candidatePrefixes) {
                    if (p && reqUrl.startsWith(p) && p !== socketPath) {
                        try {
                            ...
                            logger.info("upgrade: rewrote socket upgrade url for legacy path", {
                                original: reqUrl,
...
export function registerEngineAugmentations(engine: unknown, socketPath: string): void {
    try {
        ...
        if (typeof engineOn === "function") {
            ...
            engineOnFn.call(engine, "initial_headers", (headers: Record<string, string>, req: unknown) => {
                try {
                    const incomingOrigin = getOriginFromReq(req);
                    const allowAllOrigins =
                        SERVER_ENV.NODE_ENV !== "production" && !(SERVER_ENV.CORS_ORIGINS || SERVER_ENV.CLIENT_URL);
                    ...
                    if (typeof incomingOrigin === "string" && incomingOrigin.length > 0) {
                        ...
                        else if (allowExtensionOrigins && incomingOrigin.startsWith("chrome-extension://"))
                    ...
                    if (setOrigin) {
                        ...
                        headers["Vary"] = "Origin";
                    } else if (typeof incomingOrigin === "undefined" && SERVER_ENV.NODE_ENV === "production") {
                        const reqUrlForLog =
                            isObject(req) && typeof (req as { url?: unknown }).url === "string"
                                ? (req as { url?: unknown }).url
                                : undefined;
                        logger.warn("socket polling handshake missing Origin in production; not adding ACAO", {
                            url: reqUrlForLog,
                        });
                    }
                } catch (err: unknown) {
    ...
    try {
        ...
        if (isObject(httpServer)) {
            ...
            if (typeof maybeHttpOn === "function") {
                const httpOnFn = maybeHttpOn as (ev: string, handler: (req: unknown, res: unknown) => void) => void;
                const reqHandler = (req: unknown, res: unknown) => {
                    try {
                        ...
                        const candidatePrefixes = [socketPath, "/socket.io", "/api/socket.io"].filter(Boolean);
                        const reqUrl = typeof reqObj.url === "string" ? reqObj.url : undefined;
                        if (!reqUrl) return;
                        const matchesPrefix = candidatePrefixes.some((p) => reqUrl.startsWith(p));
                        if (!matchesPrefix) return;
                        let incomingOrigin: string | undefined;
                        if (isObject(reqObj.headers) && typeof reqObj.headers.origin === "string")
                            incomingOrigin = reqObj.headers.origin;
                        if (typeof incomingOrigin !== "string" || incomingOrigin.trim().length === 0) {
                            if (SERVER_ENV.NODE_ENV !== "production") {
                                try {
                                    const setHeader = (res as Record<string, unknown>)["setHeader"];
                                    if (typeof setHeader === "function") {
                                        const setHeaderFn = setHeader as (k: string, v: string) => void;
                                        setHeaderFn.call(res, "Access-Control-Allow-Origin", "null");
                                        setHeaderFn.call(res, "Access-Control-Allow-Credentials", "true");
                                        setHeaderFn.call(res, "Vary", "Origin");
                                    }
                                } catch {
                                    // ignore
                                }
                                logger.info("engine http request: patched ACAO for no-origin request", {
                                    url: reqUrl,
                        ...
                        if (SERVER_ENV.NODE_ENV !== "production") {
                            try {
                                ...
                                logger.info("engine http request for socket path", {
                                    url: reqObj.url,
                                    method: reqObj.method,
                                    origin: isObject(rh) && typeof rh.origin === "string" ? rh.origin : null,
                                    referer: isObject(rh) && typeof rh.referer === "string" ? rh.referer : null,
                                    cookie: isObject(rh) && typeof rh.cookie === "string" ? "[REDACTED]" : undefined,
                                    ua: isObject(rh) && rh["user-agent"] ? rh["user-agent"] : null,
                                    ts: new Date().toISOString(),
                                });
                            } catch (err: unknown) {
                ...
                try {
                    ...
                    if (isObject(req)) {
                        ...
                        logger.info("engine connection established", {
                            ...
                            origin:
                                isObject(headers) && typeof headers["origin"] === "string" ? headers["origin"] : null,
                            referer:
                                isObject(headers) && typeof headers["referer"] === "string" ? headers["referer"] : null,
                            ua: isObject(headers) && headers["user-agent"] ? headers["user-agent"] : null,
                            ts: new Date().toISOString(),
                        });
                    }
                    ...
                    if (isObject(conn)) {
                        ...
                        if (typeof maybeOn === "function") {
                            const connOnFn = maybeOn as (ev: string, handler: (...a: unknown[]) => void) => void;
                            ...
                            const connOnCloseFn = maybeOn as (ev: string, handler: (reason: unknown) => void) => void;
