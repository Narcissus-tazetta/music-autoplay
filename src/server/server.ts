...
// Respond to DevTools app-specific probe requests to avoid noisy 404s/log spam.
// Chrome DevTools sometimes requests /.well-known/appspecific/... which isn't a
// real route for our app. Return 204 No Content so the request is ignored.
app.use((req, res, next) => {
    try {
        if (typeof req.path === "string" && req.path.startsWith("/.well-known/")) {
            // Quick pattern: /.well-known/appspecific/*
            if (req.path.includes("appspecific") || req.path.includes("devtools")) {
                res.status(204).end();
                return;
            }
        }
