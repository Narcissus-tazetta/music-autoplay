...
server.on("error", (err) => {
    if ((err as NodeJS.ErrnoException).code === "EADDRINUSE") {
        logger.error(`Port ${port} is already in use. Please choose a different port or stop the existing process.`);
        process.exit(1);
    } else {
        ...
        build: viteDevServer
            ? async () => {
                  try {
                      ...
                      console.warn("vite ssrLoadModule failed, falling back to built server build", err);
