...
export function validateMusicData(music: Music): {
    ...
    if (!music.url || typeof music.url !== 'string') return { isValid: false, error: 'URLが必要です' };

    if (!validateYouTubeUrl(music.url)) return { isValid: false, error: '有効なYouTube URLを入力してください' };

    if (music.url.length > 2000) return { isValid: false, error: 'URLが長すぎます' };

    const title = sanitizeString(music.title, 200);
    if (music.title && music.title.length > 200) log.warn(`⚠️ Title truncated: "${music.title.slice(0, 50)}..."`);
...
export function validateUrlForDelete(url: string | { url: string }): {
    ...
    if (typeof urlString !== 'string') return { isValid: false, error: '無効なURL形式です' };

    if (!validateYouTubeUrl(urlString)) return { isValid: false, error: '有効なYouTube URLを入力してください' };
...
export function logValidationEvent(
    ...
    if (error) {
        log.warn(`⚠️ Validation failed [${eventType}] from ${shortId}...: ${error}`);
        if (data) log.debug(`📋 Invalid data: ${JSON.stringify(data).slice(0, 100)}`);
    } else {
