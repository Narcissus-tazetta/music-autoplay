...
interface AdminState {
  ...
  logout: () => void;
  updateSettings: (
    values: Partial<Omit<AdminState, "updateSettings" | "logout">>,
  ) => void;
}

// デフォルト値を一元管理
export const DEFAULT_ADMIN_STATE: Omit<
  AdminState,
  "setIsAdmin" | "logout" | "updateSettings"
> = {
  isAdmin: false,
};
  ...
  persist(
    (set) => {
      ...
      (Object.keys(base) as (keyof typeof base)[]).forEach((key) => {
        ...
        setters[setterName] = (value: (typeof base)[key]) => {
