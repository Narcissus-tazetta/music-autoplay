...
// No direct Recharts tooltip types required here; keep local thin types instead.
      ...
      <div className="grid gap-1.5">
        {payload.map((item: TooltipItem, index: number) => {
          ...
          const indicatorColor =
            color || (payloadObj?.fill as string) || item.color;
...
function ChartLegendContent({
  ...
  payload?: LegendItem[];
  verticalAlign?: "bottom" | "top" | "middle" | "left" | "right";
  hideIcon?: boolean;
  nameKey?: string;
}) {
  const { config } = useChart();
