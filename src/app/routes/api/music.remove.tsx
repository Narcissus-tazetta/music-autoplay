...
export const action = async ({ request, context }: ActionFunctionArgs<ServerContext>) => {
    ...
    try {
        ...
        if (isAdminRequest) {
            requesterHash = createHash("sha256").update(String(SERVER_ENV.ADMIN_SECRET)).digest("hex");
            logger.info("Using admin hash for deletion");
        } else {
            if (!user?.id) {
                return Response.json({ success: false, error: "ユーザーIDが見つかりません" }, { status: 401 });
            }
            requesterHash = createHash("sha256").update(String(user.id)).digest("hex");
            logger.info("Using user hash for deletion");
        }

        const maybe = context.io.removeMusic(submission.value.url, requesterHash) as unknown;
        let result: unknown;
        ...
        if (result && typeof result === "object" && "formErrors" in (result as Record<string, unknown>)) {
            const r = result as { formErrors?: unknown };
            if (Array.isArray(r.formErrors)) {
                return Response.json({ success: false, error: r.formErrors.join(" ") }, { status: 403 });
            }
        }
