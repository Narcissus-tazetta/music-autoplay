...
export const loader = async ({ request }: LoaderFunctionArgs) => {
    const res = await safeExecuteAsync(async () => {
        ...
        if (user == null || !user.id) return { __status: 204 } as { __status: number };
        const stored = defaultSettingsStore.load(user.id);
        return stored ? stored.value : {};
    });

    if (!res.ok) return respondWithResult(makeErr({ message: Object.prototype.toString.call(res.error) }));
...
export const action = async ({ request }: LoaderFunctionArgs) => {
    const res = await safeExecuteAsync(async () => {
        ...
        if (data == null || typeof data !== 'object') return { __status: 400 } as { __status: number };
