...
export class GoogleOIDCStrategy<User> extends OAuth2Strategy<User> {
    ...
    constructor(
        ...
        super({
            ...
            const profile = await this.verifyIdToken(tokens.idToken(), options.clientId);

            return await verify(profile);
        });
    }

    async verifyIdToken(idToken: string, clientId: string): Promise<GoogleOIDCClaims> {
        try {
            const jwks = await fetch(`https://www.googleapis.com/oauth2/v3/certs`).then(res =>
            ...
            const { payload } = await jwtVerify<GoogleOIDCClaims>(idToken, publicKey, {
                issuer: 'https://accounts.google.com',
                audience: clientId,
            });

            if (!payload.sub || !payload.email) throw new Error('Invalid ID token payload');
