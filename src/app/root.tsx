        ...
        void (async () => {
            ...
            (async () => {
                const doFetchOnce = async (signal: AbortSignal) => {
                    const resp = await fetch("/api/musics", { cache: "no-store", signal });
                    if (!resp.ok) throw new Error(`status:${resp.status}`);
                    const parsed: unknown = await resp.json().catch(() => null);
                    if (typeof parsed !== "object" || parsed === null) throw new Error("invalid-json");
                    const rec = parsed as Record<string, unknown>;
                    if (!Array.isArray(rec.musics) || typeof store.setMusics !== "function")
                        throw new Error("no-musics");
                    const maybeMusics = rec.musics as unknown[];
                    const isMusic = (v: unknown): v is import("./stores/musicStore").Music => {
                ...
                for (let i = 0; i < attempts.length; i++) {
                    ...
                    try {
                        ...
                        clearTimeout(timeout);
                        if (import.meta.env.DEV)
                            console.debug("/api/musics fetch attempt failed", { attempt: i + 1, error: err });
                        if (i === attempts.length - 1) {
                            if (import.meta.env.DEV)
                                console.debug("/api/musics all attempts failed, relying on socket");
                        }
                    }
                }
            })().catch((err: unknown) => {
                if (import.meta.env.DEV) console.debug("background /api/musics task failed", err);
            });
        })().catch((err: unknown) => {
            if (import.meta.env.DEV) {
                if (err instanceof Error) console.error(err);
