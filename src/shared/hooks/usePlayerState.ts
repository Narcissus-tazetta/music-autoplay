...
export function useInterpolatedTime({
  ...
  useEffect(() => {
    ...
    const adJustEnded =
      lastIsAdvertisementRef.current === true && isAdvertisement === false;
    ...
    if (videoIdChanged || adJustEnded) {
      ...
      if (isAdvertisement !== lastIsAdvertisementRef.current)
      ...
      // 広告開始時(false→true)は状態をリセットせずrefのみ更新
      lastIsAdvertisementRef.current = isAdvertisement;
    }
  ...
  useEffect(() => {
    if (status?.type === "playing" && typeof status.currentTime === "number") {
      ...
      const seekDetected =
        timeDiff >= SEEK_THRESHOLD || lastUpdateTimestampRef.current === 0;
  ...
  useEffect(() => {
    ...
    const animate = (): void => {
      ...
      const isPlayingNow =
        status.type === "playing" && !effectivePausedRef.current;
      const newTime = isPlayingNow
        ? baseTimeRef.current + elapsed
        : baseTimeRef.current;
      const clampedTime =
        duration != null ? Math.min(newTime, duration) : newTime;
