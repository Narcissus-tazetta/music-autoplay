...
export const ProgressBarSettings = ({
  ...
  return (
    <div className="flex flex-col gap-4">
        ...
        <div className="ml-4 flex flex-col gap-3 p-3 border rounded-lg border-gray-200 dark:border-gray-700">
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              />
              <span
                className={`text-sm ${mode === "dark" ? "text-white" : "text-black"}`}
              >
                年表示
              </span>
            </div>
            {showYear && (
              <select
                value={yearFormat}
                onChange={(e) => {
                  setYearFormat(e.target.value as YearFormat);
                }}
          ...
          <div className="flex items-center justify-between">
            ...
            {showMonth && (
              <select
                value={monthFormat}
                onChange={(e) => {
                  setMonthFormat(e.target.value as MonthFormat);
                }}
                className="select select-sm select-bordered max-w-xs text-right"
              >
          ...
          <div className="flex items-center justify-between">
            ...
            {showDay && (
              <select
                value={dayFormat}
                onChange={(e) => {
                  setDayFormat(e.target.value as DayFormat);
                }}
                className="select select-sm select-bordered max-w-xs text-right"
              >
          ...
          <div className="w-full flex items-center gap-2">
            ...
            {showWeekday && (
              <select
                value={weekdayFormat}
                onChange={(e) => {
                  setWeekdayFormat(e.target.value as WeekdayFormat);
                }}
                className="select select-sm select-bordered min-w-[100px] text-right"
              >
                <option value="japanese">
                  {weekdays.japanese[currentWeekday]}
                </option>
      ...
      >
        ...
        <input
          ...
          onChange={(e) => {
            ...
            if (!e.target.checked) setShowCurrentSchedule(false);
          }}
        />
      </label>
      ...
      {showRemainingText && (
        <label
          className={`flex items-center justify-between cursor-pointer py-2 ml-4 ${
            mode === "dark" ? "text-white" : "text-black"
          }`}
        >
          ...
          <input
            ...
            onChange={(e) => {
              setShowCurrentSchedule(e.target.checked);
            }}
          />
        </label>
      )}
      ...
      {showProgress && (
        <div className="flex flex-col gap-2">
          <span
            className={`block font-medium ${mode === "dark" ? "text-white" : "text-black"}`}
          >
            進捗バーの色
          </span>
