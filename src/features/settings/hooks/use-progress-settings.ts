...
export function useProgressSettings() {
  ...
  const [backgroundImageFileName, setBackgroundImageFileNameState] =
    useState<string>("");
  ...
  useEffect(() => {
    const loadBackgroundImage = async () => {
      try {
        ...
        if (
          savedBackgroundImageData &&
          savedBackgroundImageData.data &&
          isMounted.current
        ) {
          setBackgroundImageState(savedBackgroundImageData.data);
          setBackgroundImageFileNameState(
            savedBackgroundImageData.fileName || "",
          );
        }
      } catch (error) {
    ...
    try {
      ...
      if (imageData) {
        try {
          ...
          console.error(
            "localStorageへの保存も失敗しました:",
            localStorageError,
          );
          window.alert(
            "画像が大きすぎて保存できませんでした。より小さい画像を選択してください。",
          );
        }
      } else {
        localStorage.removeItem("backgroundImage");
