...
export const formatTimeRemaining = (remainingMs: number): string => {
  ...
  const formattedCentiseconds = `${centiseconds}`.padStart(2, "0");
...
export const findNextItem = (
  ...
  return schedule.items.find(
    (item) => timeToMs(item.startTime) > currentTimeMs,
  );
};
...
export const getCurrentStatus = (
  ...
  for (let i = 0; i < schedule.items.length; i++) {
    ...
    const nextItemStartMs =
      i + 1 < schedule.items.length
        ? timeToMs(schedule.items[i + 1].startTime)
        : Infinity;
