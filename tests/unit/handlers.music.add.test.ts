...
describe("createMusicHandlers.addMusic", () => {
  ...
  it("duplicate returns formErrors", async () => {
    const youtubeService: {
      ...
      getVideoDetails: () => Promise.resolve({ ok: true } as unknown),
    };
    const io = makeIo();
    const fileStore = makeFileStore();
    const musicDB = new Map<string, Music>();
    musicDB.set("dQw4w9WgXcQ", {
      id: "dQw4w9WgXcQ",
      title: "",
      channelName: "",
      channelId: "",
  ...
  it("age restricted is rejected", async () => {
    const youtubeService: {
      ...
      getVideoDetails: () =>
        Promise.resolve({ ok: true, value: { isAgeRestricted: true } }),
    };
    const io = makeIo();
    const fileStore = makeFileStore();
    const musicDB = new Map<string, Music>();
  ...
  it("fileStore returns promise and is awaited", async () => {
    const youtubeService: {
      getVideoDetails: (
        id: string,
      ) => Promise<VideoOk | { ok: false; error: string }>;
    } = {
      getVideoDetails: (id: string) => {
        void id;
        return Promise.resolve({
