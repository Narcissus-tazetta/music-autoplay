...
describe("Graceful Shutdown", () => {
    ...
    const createGracefulShutdown = (server: any, socketServer: any, fileStore: any, shutdownTimeout = 5000) => {
        return async () => {
            const forceExit = () => {
    ...
    it("should shutdown gracefully when all components close successfully", async () => {
        ...
        const gracefulShutdown = createGracefulShutdown(mockServer, mockSocketServer, mockFileStore, 5000);
    ...
    it("should handle socket server close errors gracefully", async () => {
        ...
        mockSocketServer.close = vi.fn().mockRejectedValue(new Error("Socket close failed"));
