...
async function waitForServer(page: Page, url = BASE_URL, timeout = 20000) {
    ...
    for (let i = 0; i < 10; i++) {
        try {
            ...
            await expect(page.locator('h1:has-text("楽曲リクエストフォーム")')).toBeVisible({ timeout: 5000 });
            return;
        } catch (e) {
            lastError = e;
...
test.describe("楽曲リクエストフォーム", () => {
    ...
    test("不正なYouTube URLはエラー表示", async ({ page }) => {
        ...
        await expect(page.locator("text=有効なYouTubeのURLを入力してください")).toBeVisible({ timeout: 5000 });
    });
...
test.describe("設定パネル", () => {
    ...
    test("設定パネルの表示・非表示", async ({ page }) => {
        ...
        await expect(page.locator('button[aria-label*="テーマ"]')).toBeVisible({ timeout: 5000 });
    });
});
...
test.describe("ネットワーク障害時の挙動", () => {
    ...
    test("Socket.IO接続失敗時でもUIが壊れない", async ({ page }) => {
        ...
        await page.reload();
        await expect(page.locator('h1:has-text("楽曲リクエストフォーム")')).toBeVisible({ timeout: 10000 });
        await submitYoutubeUrl(page, VALID_YOUTUBE_URL);
